"""
rq13_server.py  —  EcoMyth-VLM RQ13 Re-annotation Server
=========================================================
Drop this in your EcoMyth project root alongside phase6_server.py.
Run from project root:

    python rq13_server.py

Then open: http://localhost:8081   (different port — won't conflict)

Reads  : outputs/phase6_rq13/rq13_stories.json   (100 sampled stories)
Saves  : outputs/phase6_rq13/rq13_human_annotations.csv
HTML   : EcoMyth_Phase6_AnnotationTool_v4.html    (same tool, no changes)
GT     : outputs/phase3/species_ground_truth.json  (same ground truth)

NEVER touches outputs/phase6/phase6_annotations.csv
"""
...
STORIES_FILE   = PROJECT_ROOT / "outputs" / "phase6_rq13" / "rq13_stories.json"
GT_FILE        = PROJECT_ROOT / "outputs" / "phase3" / "species_ground_truth.json"
ANNO_FILE      = PROJECT_ROOT / "outputs" / "phase6_rq13" / "rq13_human_annotations.csv"
TOOL_HTML      = PROJECT_ROOT / "EcoMyth_Phase6_AnnotationTool_v4.html"

PORT = 8081   # different from phase6_server (8080)
    ...
    def do_GET(self):
        ...
        if path in ("", "index.html"):
            if TOOL_HTML.exists():
                ...
                self.send_text(
                    f"HTML tool not found.\n"
                    f"Expected: {TOOL_HTML}\n"
                    "Place EcoMyth_Phase6_AnnotationTool_v4.html in project root.", 404)
            return
        ...
        if path == "api/stories":
            if STORIES_FILE.exists():
                ...
                self.send_json({"ok": False,
                    "error": (
                        f"RQ13 stories not found: {STORIES_FILE}\n"
                        "Run: python sample_100.py first."
                    )}, 404)
            return
        ...
        if path == "api/annotations":
            if ANNO_FILE.exists():
                ...
                self.send_json({"ok": True, "annotations": annos, "count": len(annos)})
            else:
        ...
        if path == "api/status":
            self.send_json({
                "ok":            True,
                "mode":          "RQ13 re-annotation (100 stories)",
    ...
    def do_POST(self):
        ...
        if path == "api/save":
            try:
                ...
                print(f"  Saved [{len(existing):3d}/100] {anno['serial_number']} "
                      f"| {anno.get('model_key','?')} "
                      f"| {anno.get('format','?')} "
                      f"| {anno.get('scientific_name','?')}")
        ...
        self.send_json({"ok": False, "error": f"Unknown POST: {path}"}, 404)


def main():
    print("=" * 56)
    print("  EcoMyth-VLM RQ13 Re-annotation Server")
    print("=" * 56)
    print()

    all_ok = True
    checks = [
        (TOOL_HTML,    "Annotation HTML (required)",  True),
        (STORIES_FILE, "RQ13 stories (100 stories)",  True),
        (GT_FILE,      "Ground truth JSON",           False),
    ]
    for fpath, label, required in checks:
        exists = fpath.exists()
        icon   = "OK" if exists else ("MISSING — REQUIRED" if required else "MISSING (optional)")
    ...
    if not all_ok:
        print()
        print("  Fix missing files above, then re-run.")
        print("  Stories: python sample_100.py")
        return
    ...
    if ANNO_FILE.exists():
        ...
        print(f"  Resuming: {n}/100 already annotated")
    else:
        print(f"  Starting fresh — will save to:")
        print(f"    {ANNO_FILE}")

    print()
    print(f"  Server: http://localhost:{PORT}")
    print(f"  (Phase 6 server runs on 8080 — no conflict)")
    print()
    ...
    try:
        ...
        if ANNO_FILE.exists():
            with open(ANNO_FILE, encoding="utf-8", newline="") as f:
                n = sum(1 for _ in csv.DictReader(f))
            print(f"  Annotations saved: {n}/100 --> {ANNO_FILE}")
        print()
